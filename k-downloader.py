...
def MsgBox(text, title=TITLE, style=MB_ICONEXCLAMATION | MB_SYSTEMMODAL):
...
def is_running(name):
    ...
    return False
"""
URL = 'https://tv.naver.com/v/58959969'
URL = 'https://vimeo.com/641605882'
URL = 'https://www.youtube.com/watch?v=KmB1MJO81wQ'
URL = 'https://www.facebook.com/OddCouplesDodo/videos/5219642314759701/'
URL = 'https://vod.afreecatv.com/player/86092427'
URL = 'https://tv.kakao.com/channel/10097932/cliplink/448693411'
URL = 'https://www.youtube.com/watch?v=yZeNfaIK7Nw'    # 경복궁
URL = 'https://www.youtube.com/watch?v=MAdl-Z1JLD8' # 8K
"""
URL = 'https://www.youtube.com/watch?v=8QJhXRGtdg0'
...
# YT_DLP = os.getcwd() + '\\yt-dlp.exe'
FFMPEG = r'.\ffmpeg.exe'
YT_DLP = 'yt-dlp.exe'
YT_DLP_PATH = rf'.\{YT_DLP}'
...
class WorkerThread(Thread):
    ...
    def analyzeIt(self):
        ...
        if parent.task_done:
            ...
            wx.PostEvent(parent, ResultEvent('analyze-cancelled2'))
    ...
    def checkprogress(self):
        ...
        if s.startswith('WARNING:'):
            ...
            s = s.replace('Webpage contains broken formats (poToken experiment detected). Ignoring initial player response',
    ...
    def downloadIt(self):
        ...
        if parent.cur_video['format'][0] and parent.cur_video['format'][1]:
            format = f'{parent.cur_video["format"][0]}+{parent.cur_video["format"][1]}'
        elif parent.cur_video['format'][0]:
            format = parent.cur_video['format'][0]
        ...
        cmd = f'powershell & "{YT_DLP_PATH}" -f "{format}" "{url}" {legacyserverconnect} -P "{parent.config["download-dir"]}" -o "{outfile}"'.split() + \
              '2>&1 | % ToString | Tee-Object out.txt'.split()
        # print('task =>', parent.task, ', cmd =>', ' '.join(cmd))
        ...
        if parent.task_done:
            ...
            wx.PostEvent(parent, ResultEvent('download-cancelled2'))
...
class WorkerThread3(Thread):
    ...
    def abort(self):
        wx.PostEvent(self.parent, ResultEvent('ytdlp-cancelled'))
        self.raise_exception()
...
class WorkerThread4(Thread):
    ...
    def run(self):
        ...
        r = re.search(r"__version__ = '(.*?)'", text)
        if r:
            parent.ytdlp_latest_version = r.group(1).strip()
...
class VideoDownloader(wx.Frame):
    def __init__(self, parent):
        ...
        # 이벤트
        self.Bind(wx.EVT_MENU, self.openDir, id=101)
        self.Bind(wx.EVT_MENU, self.onremux, id=111)
        self.Bind(wx.EVT_MENU, self.onClose, id=109)
        self.Bind(wx.EVT_MENU, self.updateYtDlp, id=203)
        self.Bind(wx.EVT_MENU, self.onAbout, id=204)
        self.Bind(wx.EVT_CLOSE, self.onWindowClose)
        txtURL.Bind(wx.EVT_TEXT, self.checkUrl)
        btnPaste.Bind(wx.EVT_BUTTON, self.pasteIt)
        btnAnalyze.Bind(wx.EVT_BUTTON, self.onanalyze)
        btnOpenDir.Bind(wx.EVT_BUTTON, self.openDir2)
        btnOpen.Bind(wx.EVT_BUTTON, self.openFile)
        btnAbort.Bind(wx.EVT_BUTTON, self.abortIt)
        dvlc.Bind(wx.dataview.EVT_DATAVIEW_SELECTION_CHANGED, self.onSelFormat)
        dvlc_2.Bind(wx.dataview.EVT_DATAVIEW_SELECTION_CHANGED, self.onSelFormat_2)
        btnHelp.Bind(wx.EVT_BUTTON, self.help)
        btnPreview.Bind(wx.EVT_BUTTON, self.preview)
        btnHelp2.Bind(wx.EVT_BUTTON, self.help2)
        btnUnselect.Bind(wx.EVT_BUTTON, self.unselectIt)
        btnPreview2.Bind(wx.EVT_BUTTON, self.preview2)
        self.cbRemux.Bind(wx.EVT_CHECKBOX, self.checkRemux)
        self.btnHelp3.Bind(wx.EVT_BUTTON, self.helpIt3)
        self.btnDownload.Bind(wx.EVT_BUTTON, self.ondownload)
        self.btnDownloadDir.Bind(wx.EVT_BUTTON, self.ondownloaddir)
        self.Bind(wx.EVT_CHAR_HOOK, self.onkey)
        EVT_RESULT(self, self.OnResult)

        txtURL.SetValue(URL)
        self.ytdlp_current_version = get_version_number(YT_DLP_PATH)
        wx.CallLater(1, self.checkVersionLatest)
    ...
    def checkVersionLatest(self):
        self.worker4 = WorkerThread4(self)
        self.worker4.daemon = True
        self.worker4.start()

    def checkIntegral(self, n):
    ...
    def checkRange(self, s):
        ...
        if '.' in s:
            if s.count('.') > 1:
                ...
                if not self.checkIntegral(integral):
                    return False

                if not self.checkDecimal(decimal):
                    return False
        else:
            if not self.checkIntegral(s):
                return False

        return True

    def checkUrl(self, evt):
    ...
    def OnResult(self, evt):
        ...
        if evt.data:
            if evt.data in ['analyze-cancelled', 'analyze-cancelled2']:
                ...
                self.cancelled = True
                self.killtask(msg)
                self.restoreControls('remux')

            elif evt.data == 'ytdlp-cancelled':
                self.gauge.SetValue(0)
                msg = 'yt-dlp 업데이트 취소되었습니다.'
                self.status.SetLabel(msg)
                self.cancelled = True
                self.killtask(msg)
                self.restoreControls('ytdlp')

            elif evt.data == 'analyze-finished':
                self.restoreControls('analyze')
                self.displayFormats('table')
                #self.displayFormats('json')
                ...
                if self.no_ffmpeg:
                    ...
                    if (self.cbRemux_visible and self.config["remuxing"]):
                        wx.CallLater(1, self.remuxIt, self.outfile)
                    else:
                        self.openDir2()
    ...
    def displayFormats(self, arg):
        ...
        if arg == 'table':
            self.cur_video["id"], self.cur_video["title"], self.cur_video["duration"], self.cur_video["is_live"] = \
                self.getVideoInfo(self.formats_data['json-string'])
            ...
            L = []
            L_ = []
            print('0=>ID, 1=>EXT, 2=>RESOLUTION, 3=>FPS, 4=>CH, 5=>FILESIZE, 6=>TBR, 7=>PROTO, 8=>VCODEC, 9=>VBR, 10=>ACODEC, 11=>ABR, 12=>ASR, 13=>MORE INFO, 14=>VIDEO(O/X), 15=>AUDIO(O/X), 16=>url')
            ...
            for line in self.lines:
                ...
                url = self.getVideoUrl(self.formats_data['json-string'], l[0])
                l2.append(url)
                l += l2
                # print(l)
                L.append(l)
            ...
            L2 = []
            for l in L:
                if l[1] in ['mp4', 'webm'] and l[7] in ['http', 'https'] and l[14] == 'O':   # 1=>ext, 7=>proto, 14=>video(O/X)
            ...
            if True:
                for l in L:
                    if self.host == 'youtube':
                        if l[1] in ['mp4', 'webm'] and l[7] == 'm3u8' and l[14] == 'O':   # 1=>ext, 7=>proto, 14=>video(O/X), 15=>audio(O/X)
                        ...
                        if 'fastly' in l[0]:
                            if l[1] in ['mp4', 'webm'] and l[7] == 'm3u8' and l[14] == 'O':   # 1=>ext, 7=>proto, 14=>video(O/X)
                                print("v protocol:m3u8", l)
                                L2.append(l)
                    else:
                        if l[1] in ['mp4', 'webm'] and l[7] == 'm3u8' and l[14] == 'O':   # 1=>ext, 7=>proto, 14=>video(O/X)
            ...
            if L2:
                L2.sort(key=lambda col: (2 if col[7] in ['http', 'https'] else (1 if col[7] == 'm3u8' else 0), 1 if col[1]=='mp4' else 0, int(col[2].split('x')[1]) if 'x' in col[2] else (0 if col[2]=='sd' else 1), int(col[2].split('x')[0]) if 'x' in col[2] else (0 if col[2]=='sd' else 1),
            ...
            if no_audio:
                L2 = []
                for l in L:
                    if l[1] in ['m4a', 'webm'] and l[7] in ['http', 'https'] and l[15] == 'O':   # 1=>ext, 7=>proto, 15=>audio(O/X)
                ...
                if True:
                    for l in L:
                        if self.host == 'vimeo':
                            if 'fastly' in l[0]:
                                if l[1] == 'mp4' and l[7] == 'm3u8' and l[15] == 'O':   # 1=>ext, 7=>proto, 11=>abr, 15=>audio(O/X)
                                    if 'high' in l[0]:
                                        l[11] = '253k'
                                    else:
                                        l[11] = '125k'

                                    abr = 253375 if l[11] == '253k' else (125375 if l[11] == '125k' else int(l[11][:-1]) * 1000)
                            ...
                            if l[1] == 'mp4' and l[7] == 'm3u8' and l[11] != '' and l[15] == 'O':   # 1=>ext, 7=>proto, 11=>abr,15=>audio(O/X)
                                print("a protocol:m3u8", l)
                                L2.append(l)
                ...
                if L2:
                    L2.sort(key=lambda col: (2 if col[7] in ['http', 'https'] else (1 if col[7] == 'm3u8' else 0), 1 if col[1] in ['m4a', 'mp4'] else 0, col[11], col[5], 1 if 'DRC' in col[13] else 0), reverse=True) # 1=>ext, 5=>filesize, 11=>abr, 13=>more info
                    L3_2 += L2

            for l in L3:
                l = [f'{l[2]} ( {l[1]}, {l[6].strip()} )', l[5]] if l[6] else [f'{l[2]} ( {l[1]} )', l[5]]  # 1=>ext, 2=>resolution, 5=>filesize, 6=>tbr
                # print(l)
            ...
            self.L4 = []
            for l in L3_2:
                self.L4.append([l[0], l[1], l[4], l[5], l[10], l[11], l[12], l[16], l[7]])  #  0=>id, 1=>ext, 4=>ch, 5=>filesize, 10=>acodec, 11=>abr, 12=>asr, 16=>url, 7=>proto
        ...
        """
            ...
            data = self.formats_data['table']
            # print(data)
            self.cur_video['id'] = data['id']
            # self.cur_video['title'] = data["title"].replace("|", "#")
            self.cur_video['title'] = data["title"].replace("|", "#").encode().decode('unicode-escape')
            L = []
            L_ = []
            formats = None
            if 'formats' in data:
                ...
                formats = data['entries'][0]['formats']
            else:
                msg = '동영상 URL이 맞게 입력되었는지 확인해주세요.'
                self.status.SetLabel(msg)
                return

            for el in formats:
                # print(el)
                if el['protocol'] == 'http_dash_segments':
                    continue

                if self.host == 'vimeo':
                    if (not 'vcodec' in el or not el['vcodec'].startswith('avc1')
                            or 'abr' not in el):
                        continue

                id = el['format_id']
                ext = 'm3u8' if el['url'].endswith('.m3u8') else el['ext']
                width = el['width'] if 'width' in el else ''
                height = el['height'] if 'height' in el else ''
                if self.host == 'facebook':
                    if id == 'sd':
                        resolution = '426x426'
                    elif id == 'hd':
                ...
                fps = str(el['fps']) if 'fps' in el else ''
                filesize = ''
                if self.host not in ['naver', 'facebook']:
                    if 'filesize' in el and el['filesize'] != None:
                        filesize = '{:.2f}MiB'.format(el['filesize']/1048576,)
                    elif 'filesize_approx' in el and el['filesize_approx'] != None:
                        filesize = '{:.2f}MiB'.format(el['filesize_approx']/1048576,)

                tbr = '{:.0f}k'.format(el['tbr'],) if 'tbr' in el and el['tbr'] not in ['none', None] else ''
                if self.host == 'facebook':
                    if id in ['sd', 'hd']:
                        tbr = '?k'
                ...
                vcodec = ''
                if 'VCODEC' in el:
                    vcodec = el['vcodec'] if el['vcodec'] not in ['none', None] else ''
                else:
                    if self.host == 'facebook':
                        vcodec = 'avc1'

                vbr = '{:.0f}k'.format(el['vbr'],) if 'vbr' in el and el['vbr'] not in ['none', None] else ''
                acodec = ''
                if 'acodec' in el:
                    acodec = el['acodec'] if el['acodec'] not in ['none', None] else ''

                abr = '{:.0f}k'.format(el['abr'],) if 'abr' in el and el['abr'] not in ['none', None] else ''
                asr = f'{el["asr"]}Hz' if 'asr' in el and el['asr'] not in ['none', None] else ''
                x = el['format_note'] if 'format_note' in el and el['format_note'] not in ['none', None] else ''
                y = f'{ext}_dash' if not (vcodec and acodec) else ''
                etc = f'{x}, {y}' if x and y else x
                if self.host in ['afreecatv', 'naver']:
                    v = 'O'
                    a = 'O'
                elif self.host in ['facebook']:
                    v = 'O' if resolution != 'audio only' else 'X'
                    a = 'O' if acodec or id in ['sd', 'hd'] else 'X'
                else:
                    v = 'O' if vcodec else 'X'
                    a = 'O' if acodec else 'X'

                self.cur_video['m3u8'] = False
                url = ''
                if el['url'] not in ['none', None]:
                    if el['url'].endswith('.m3u8'):
                        url = el['manifest_url'] if 'manifest_url' in el else el['url']
                        self.cur_video['m3u8'] = True
                    else:
                        url = el['url']

                self.cur_video['url'] = url
                if not self.wtxt:
                    self.wtxt = re.search(r'(?<=//).+?(?=/)', url).group(0)

                l = [id, ext, resolution, fps, filesize, tbr, proto, vcodec, vbr, acodec, abr, asr, etc, v, a, url]
                # print(l)
                if self.host == 'vimeo':
                    l_ = l[1:-1]
                    if l_ not in L_:
            ...
            if self.host == 'naver':
                ...
                if L2:
                    L2.sort(key=lambda col: (int(col[2].split('x')[1]), int(col[2].split('x')[0]), col[3], col[4], col[9]), reverse=True)
                    L3 += L2
            else:
                L2 = []
                for l in L:
                    if l[1] == 'mp4' and l[13] == 'O':
                        L2.append(l)
                ...
                L2 = []
                for l in L:
                    if l[1] == 'mp4' and l[13] == 'X':
                        L2.append(l)

                if L2:
                    L2.sort(key=lambda col: (col[3], col[4], col[9]), reverse=True)
                    L3 += L2
            ...
            L2 = []
            for l in L:
                if l[1] == 'webm' and l[13] == 'O':
                    if not L3_2 and l[14] == 'X':
                        continue
                    L2.append(l)
            ...
            for l in L3:
                # print(l)
                l = [f'{l[2]} ( {l[1]}, {l[5]} )', l[4]] if l[5] else [f'{l[2]} ( {l[1]} )', l[4]]
                self.dvlc.AppendItem(l)

            self.dvlc.Layout()
            msg = f'동영상 {self.cur_video["id"]}: {len(L3)}개 포맷'
            self.status.SetLabel(msg)
            self.gauge.SetValue(self.gauge.GetRange())

            self.L4 = []
            for l in L3_2:
                # print([l[0], l[1], l[5], l[4], l[15]])
                self.L4.append([l[0], l[1], l[5], l[4], l[15]])
                drc = 'DRC' if l[0].endswith('-drc') else ''
                self.dvlc_2.AppendItem([l[1], l[5], l[4], drc])

            if self.is_live:
                message = '라이브 스트림 MPEG-TS(HLS)\n\n '
                wx.MessageBox(message, TITLE, wx.ICON_EXCLAMATION | wx.OK)
    ...
    def getVideoUrl(self, data, format_id):
    ...
    def analyzeData(self):
        ...
        self.pos = {}
        keys = ['ID', 'EXT', 'RESOLUTION', 'FPS', 'CH', 'FILESIZE', 'TBR', 'PROTO', 'VCODEC', 'VBR', 'ACODEC', 'ABR', 'ASR', 'MORE INFO']
        for key in keys:
            self.pos[key] = [-1, -1]
    ...
    def dvlc_init(self):
        ...
        wx.CallLater(1, self.selFormat)
        self.dvlc.SetFocus()

    def selFormat(self):
        self.onSelFormat()

    def displayFormatsFacebook(self, data):
        pass

    def displayFormatsEBS(self, url):
        msg = '[ebs] ' + url + ': 웹페이지 다운로드'
        self.status.SetLabel(msg)
        src = requests.get(url).text
        m = re.search(r'(?<=<meta property="og:title" content=").*(?=")', src)
        title = m.group(0) if m else ''
        self.cur_video['title'] = title.replace('|', '#')
        ...
        self.status.SetLabel(msg)
        L = []
        m = re.findall(r'label: .*wstrotu.ebs.co.kr.*[\w?.=]', src)
        if m:
            for i in range(len(m)):
                l = m[i].split(',')
                # print(l)
                m2 = re.search(r'(?<=label: \').*(?=\()', l[0])
                resolution = m2.group(0) if m2 else ''
                m2 = re.search(r'(?<=\().*(?=\))', l[0])
                tbr = m2.group(0) if m2 else ''
                # print(tbr)
                m2 = re.search(r'https://wstrotu.ebs.co.kr.*[\w?.=]', l[1])
                ...
                v = 'O'
                a = 'O'
                L.append([id, ext, resolution, '', '', tbr, '', '', '', '', '', '', '', v, a, url])

        self.L3 = L3 = []
        L2 = []
        for l in L:
            if l[1] == 'mp4' and 'x' in l[2]:
                L2.append(l)
        ...
        for l in L3:
            if not l[7] and l[9]:
                self.L4.append([l[0], l[1], l[5], l[4]])
                drc = 'DRC' if l[0].endswith('-drc') else ''
                self.dvlc_2.AppendItem([l[1], l[5], l[4], drc])

    def onSelFormat(self, evt=None):
        if evt:
            self.cur_dvlc = 1
        ...
        if self.worker:
            ...
            self.cur_video['format'][0 if self.L3[row][8] else 1] = format_id = self.L3[row][0]
            self.dvlc_3.DeleteAllItems()
            ...
            if self.L3[row][15] == 'X':
                self.dvlc_2.Enable()
                self.displayFormats2()
            else:
                self.dvlc_2.Disable()
                self.inner3.Hide(self.bsizer2)
                self.inner3.Layout()

    def onSelFormat_2(self, evt=None):
        if evt:
            self.cur_dvlc = 2
        ...
        if self.worker:
            ...
            self.cur_video['format'][1] = format_id = self.L4[row][0]
    ...
    def helpIt3(self, evt):
        dlg = Help(self, 3)
        dlg.ShowModal()
        dlg.Destroy()

    def unselectIt(self, evt):
        self.cur_video['format'][1] = None
        row = self.dvlc_2.GetSelectedRow()
    ...
    def preview2(self, evt):
        if 'format' not in self.cur_video:
            return

        pids = self.enumProcs()
        for pid in pids:
            self.enumProcWnds(pid)
    ...
    def ondownloaddir(self, evt=None):
    ...
    def saveControls(self, arg=None):
    ...
    def restoreControls(self, arg=None):
